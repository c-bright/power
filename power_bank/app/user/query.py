...
def get_rent_history():
    ...
    username = (g.current_user or {}).get("username") or ""
    # 分页参数
    try:
    ...
    try:
        db = get_db()
        # 构建基础查询条件
        sql_where = "WHERE 1=1 AND username = %s"
        params = [username]
...
def handle_return_and_settle():
    ...
    try:
        ...
        SELECT rent_time
        FROM rent_record
        WHERE order_no = %s AND username = %s AND status = 'renting'
        """
        db.execute(check_sql, (order_no, username))
        record = db.fetchone()
        ...
        """
        db.execute(update_record_sql, (return_time, total_cost, order_no, username))
        ...
        if not results:
            ...
            for row in results:
                ...
                notifications.append({
                    ...
                    #  保持 content 简洁，方便前端在 notification 横幅展示
