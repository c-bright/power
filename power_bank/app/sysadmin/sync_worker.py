...
class SuperSyncWorker:
    ...
    def _safe_cal(self, chain_addr):

        from app.sysadmin.test.test_admin_manager import AdminManagerTestClient
        api = AdminManagerTestClient()
        return api.get_admin_by_addr(chain_addr)
        ...
        with self.db.cursor() as cursor:
            for i in range(count):
                try:
                    ...
                    actual_data = self._safe_cal(chain_addr)
                    ...
                    deleted_flag = str(actual_seq[7]).lower() == "true"
                    if deleted_flag:
                        cursor.execute(
                            "SELECT id FROM admin WHERE address = %s", (chain_addr,)
                        )
                        exists = cursor.fetchone()

                        if exists:
                            cursor.execute(
                                "DELETE FROM admin WHERE address = %s", (chain_addr,)
                            )
